import { DollarSign, Eye, MousePointer, Target, CreditCard, TrendingUp, ArrowUp, ArrowDown, Users, Percent, Zap, Radio, Link2, Activity } from "lucide-react";
...
const CARDS: CardDef[] = [
	...
	{ key: "roas", label: "AVG. ROAS", icon: TrendingUp, color: "text-pink-600", bg: "bg-pink-50", format: "roas", row: 1 },
	// Row 2 — secondary metrics
	{ key: "reach", label: "REACH", icon: Radio, color: "text-cyan-600", bg: "bg-cyan-50", format: "number", row: 2 },
	{ key: "frequency", label: "FREQUENCY", icon: Activity, color: "text-slate-600", bg: "bg-slate-50", format: "decimal", row: 2 },
	{ key: "link_clicks", label: "LINK CLICKS", icon: Link2, color: "text-sky-600", bg: "bg-sky-50", format: "number", row: 2 },
	...
	{ key: "active_ads", label: "ACTIVE ADS", icon: Users, color: "text-rose-600", bg: "bg-rose-50", format: "number", row: 2 },
];
...
function KpiCard({ card: c, totals, totalsPrev, currency }: { card: CardDef; totals: FbAdsTotals; totalsPrev: FbAdsTotals | null; currency: string }) {
	const val = (totals[c.key] as number) ?? 0;
...
function KpiCardMini({ card: c, totals, totalsPrev, currency }: { card: CardDef; totals: FbAdsTotals; totalsPrev: FbAdsTotals | null; currency: string }) {
	const val = (totals[c.key] as number) ?? 0;
	const prevVal = totalsPrev ? (totalsPrev[c.key] as number) : undefined;
	const d = delta(val, prevVal);
	const Icon = c.icon;
