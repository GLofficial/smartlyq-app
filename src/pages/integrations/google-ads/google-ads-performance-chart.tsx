...
import type { GoogleAdsTimeseriesPoint } from "./google-ads-types";

interface PerformanceChartProps {
	timeseries: GoogleAdsTimeseriesPoint[];
	timeseriesPrev?: GoogleAdsTimeseriesPoint[] | null;
	currency: string;
}

type Metric = "spend" | "impressions" | "conversions";

const METRIC_CONFIG: Record<Metric, { label: string; format: (v: number, c: string) => string }> = {
	spend: { label: "Spend", format: (v, c) => fmtMoney(v, c) },
	impressions: { label: "Impressions", format: (v) => fmtNum(v) },
	conversions: { label: "Conversions", format: (v) => fmtNum(v) },
};
...
export function GoogleAdsPerformanceChart({ timeseries, timeseriesPrev, currency }: PerformanceChartProps) {
		...
		<div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5">
			...
			<ResponsiveContainer width="100%" height={300}>
				<ComposedChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
					<defs>
						<linearGradient id="gadsBarGrad" x1="0" y1="0" x2="0" y2="1">
					...
					<Bar yAxisId="left" dataKey={barMetric} fill="url(#gadsBarGrad)" radius={[3, 3, 0, 0]} maxBarSize={24} />
