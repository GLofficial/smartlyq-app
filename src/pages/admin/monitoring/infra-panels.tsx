			...
			{(data.tables?.length ?? 0) > 0 && (
				<div className="mt-4">
					...
					<div className="space-y-1">
						{data.tables.slice(0, 8).map((t: Record<string, unknown>, i: number) => (
							<div key={String(t?.TABLE_NAME ?? t?.name ?? i)} className="flex items-center justify-between text-xs py-1 border-b border-[var(--border)] last:border-0">
								<span className="font-mono text-[var(--foreground)]">{String(t?.TABLE_NAME ?? t?.name ?? "--")}</span>
								<span className="text-[var(--muted-foreground)]">{Number(t?.TABLE_ROWS ?? t?.rows ?? 0).toLocaleString()} rows / {Number(t?.data_mb ?? 0).toFixed(1)} MB</span>
							</div>
						))}
					</div>
				</div>
