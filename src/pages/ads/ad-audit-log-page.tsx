...
export function AdAuditLogPage() {
	...
	const entries = (data?.entries ?? []).filter(
		(e) => !search || e.entity.toLowerCase().includes(search.toLowerCase()) || e.action.toLowerCase().includes(search.toLowerCase())
	);
	...
	return (
		<div className="space-y-5">
			...
			<div className="relative w-72">
				...
				<Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search entries..." className="pl-9 h-9 text-sm" />
			</div>
										...
										return (
											<tr key={e.id} className="border-b border-[var(--border)] hover:bg-[var(--muted)]/30 transition-colors">
												...
												<td className="px-4 py-3">
													<span className="rounded bg-[var(--muted)] px-2 py-0.5 text-xs capitalize">{parts[0] || "—"}</span>
												</td>
												<td className="px-4 py-3 font-medium text-[var(--foreground)]">{parts[1] || "—"}</td>
												<td className="px-4 py-3 text-[var(--muted-foreground)] max-w-xs truncate">{e.details || "—"}</td>
												<td className="px-4 py-3">
													<span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
														(e as any).environment === "production" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
													}`}>{(e as any).environment || "Production"}</span>
												</td>
												<td className="px-4 py-3">
...
function ActionBadge({ action }: { action: string }) {
	const styles: Record<string, string> = {
		created: "bg-emerald-100 text-emerald-700", sync: "bg-blue-100 text-blue-700",
		updated: "bg-blue-100 text-blue-700", deleted: "bg-red-100 text-red-700",
		paused: "bg-amber-100 text-amber-700", activated: "bg-emerald-100 text-emerald-700",
	};
	return (
		<span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${styles[action.toLowerCase()] ?? "bg-gray-100 text-gray-600"}`}>
			<span className="text-xs">&#9679;</span> {action}
		</span>
	);
}
