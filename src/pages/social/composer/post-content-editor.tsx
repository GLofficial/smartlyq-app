...
import {
  Bold, Italic, Smile, Image, Hash, Link2, Sparkles,
  ChevronDown, FileText, ImageIcon, Video,
} from "lucide-react";
...
for (let i = 0; i < 10; i++) {
  BOLD_MAP[String(i)] = boldDigitsArr[i]!;
}
...
export function PostContentEditor({
  ...
  // The textarea currently in focus
  const activeTextarea = useCallback(
    () => (customizeChannel ? customRef.current : mainRef.current),
    [customizeChannel],
  );

  // Apply unicode formatting to the current selection
  ...
  // Resolve the lowest char limit across selected platforms
  const activeCharLimit =
    selectedPlatforms.length > 0
      ? Math.min(...selectedPlatforms.map((id) => PLATFORM_CHAR_LIMITS[id] ?? 2200))
      : 2200;

  // Current customize-tab platform
  ...
  // -----------------------------------------------------------------------
  // Toolbar (shared between both modes)
  // -----------------------------------------------------------------------
  const toolbar = (
    <div className="flex items-center gap-0.5 px-4 py-2">
        ...
        {showAiDropdown && (
          <>
            ...
            <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-lg shadow-lg py-1 z-20 min-w-[150px]">
              <button
                onClick={() => { setShowAiDropdown(false); onOpenAiText?.(); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground hover:bg-muted transition-colors"
              >
                <FileText className="w-4 h-4 text-primary" /> AI Text
              </button>
              <button
                onClick={() => { setShowAiDropdown(false); onOpenAiImage?.(); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground hover:bg-muted transition-colors"
              >
                <ImageIcon className="w-4 h-4 text-primary" /> AI Image
              </button>
              <button
                onClick={() => { setShowAiDropdown(false); onOpenAiVideo?.(); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-foreground hover:bg-muted transition-colors"
              >
      ...
      {/* Bold / Italic */}
      <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Bold" onClick={() => applyFormatting(toUnicodeBold)}>
        <Bold className="w-4 h-4" />
      </Button>
      <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Italic" onClick={() => applyFormatting(toUnicodeItalic)}>
      ...
      {/* Emoji */}
      <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Emoji" onClick={onOpenEmoji}>
        <Smile className="w-4 h-4" />
      </Button>

      {/* Media upload */}
      <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Add Image" onClick={onOpenMediaUpload}>
      ...
      <Button
        size="sm"
        variant="ghost"
        className="h-8 rounded-md px-3 text-xs font-semibold text-primary-foreground bg-gradient-to-r from-[hsl(262,60%,55%)] to-[hsl(180,60%,45%)] hover:opacity-90 ml-0.5"
      >
        Canva
      </Button>

      {/* Hashtag */}
      <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Hashtags">
        <Hash className="w-4 h-4" />
      </Button>

      {/* Link */}
      <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="Link">
    ...
    <div className="bg-card rounded-lg border border-border">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-2">
        ...
        <label className="flex items-center gap-2 text-xs text-muted-foreground cursor-pointer">
          ...
          <div
            role="switch"
            aria-checked={customizeChannel}
            onClick={() => onCustomizeChannelChange(!customizeChannel)}
            className={cn(
              "w-9 h-5 rounded-full relative transition-colors cursor-pointer",
              customizeChannel ? "bg-primary" : "bg-muted",
            )}
          >
            <div className={cn(
              "absolute top-0.5 w-4 h-4 rounded-full bg-card shadow transition-transform",
              customizeChannel ? "translate-x-4" : "translate-x-0.5",
            )} />
          </div>
        </label>
      </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {selectedPlatforms.map((pid) => {
                ...
                return (
                  ...
                  >
                    <PlatformBrandIcon platformId={pid} size={12} />
                    {brand.label}
                  </button>
                );
              })}
            ...
            <div className="px-4 pb-3">
              <Textarea
                ...
                onChange={(e) => {
                  ...
                  if (activePid === selectedPlatforms[0]) onContentChange(e.target.value);
                }}
                placeholder={`Enter your ${PLATFORM_BRANDS[activePid]?.label ?? ""} content...`}
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Char limit: {customizeValue.length} / {customizeLimit}
              </div>
            </div>
          </div>
        </>
