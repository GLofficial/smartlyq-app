...
import { ChevronLeft, ChevronRight, MessageSquare, Send, Search, Archive } from "lucide-react";
...
export function InboxPage() {
	...
	// Auto-scroll on thread update
	useEffect(() => {
		if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
	}, [thread?.messages?.length]);

	const conversations = (data?.conversations ?? []).filter((c) =>
		!search.trim() || c.participant_name.toLowerCase().includes(search.toLowerCase()) || c.snippet.toLowerCase().includes(search.toLowerCase()),
	);
				...
				<Card className="flex flex-col overflow-hidden">
					<div className="p-3 border-b border-[var(--border)] space-y-2">
						<div className="flex items-center justify-between">
							<span className="text-sm font-semibold">{data?.total ?? 0} conversations</span>
							{data && data.pages > 1 && (
