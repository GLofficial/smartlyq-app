...
export function VideoGeneratorPage() {
	...
	return (
		<div className="space-y-6">
			...
			<Card>
				<CardHeader><CardTitle className="text-base">Text to Video</CardTitle></CardHeader>
				<CardContent>
					<div className="flex gap-3">
						<Input placeholder="Describe the video..." value={prompt} onChange={(e) => setPrompt(e.target.value)} className="flex-1" />
						<Button disabled={gen.isPending} onClick={() => {
							if (!prompt.trim()) { toast.error("Enter a prompt."); return; }
							gen.mutate({ prompt, type: "text_video" }, {
								onSuccess: (d) => { toast.success(d.message); setPrompt(""); },
								onError: (e) => toast.error((e as { message?: string })?.message ?? "Failed."),
							});
						}}><Wand2 size={16} /> {gen.isPending ? "Generating..." : "Generate"}</Button>
					</div>
				</CardContent>
			</Card>
			<Card>
