...
import { ChevronLeft, ChevronRight, Image, Megaphone, Wand2 } from "lucide-react";
...
import { useImageConfig } from "@/api/tools";
...
const ASPECT_OPTIONS: { value: Aspect; label: string; icon: string }[] = [
...
export function ImageGeneratorPage() {
	...
	const activeStyleData = styles.find((s) => s.value === style);
	...
	const handleGenerate = async () => {
		...
		try {
			const body: Record<string, unknown> = {
				model: activeModel,
				aspect_ratio: aspect,
				style_prompt: activeStyleData?.prompt ?? "",
			};
			if (tab === "image") {
				body.prompt = prompt;
			} else {
				...
				body.ad_use_case = ad.use_case;
				body.ad_objective = ad.objective;
				body.ad_audience_temp = ad.audience_temp;
				body.ad_visual_angle = ad.visual_angle;
				body.ad_product_name = ad.product_name;
				body.ad_offer = ad.offer;
				body.ad_audience = ad.audience;
				body.ad_placement = ad.placement;
			}
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader className="pb-3">
					<div className="flex gap-1 p-1 bg-muted rounded-xl">
						<button
							...
							className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-sm font-medium transition-all ${tab === "image" ? "bg-background shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}
						>
							<Image size={15} /> Image
						</button>
						<button
							onClick={() => setTab("ad")}
							className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-sm font-medium transition-all ${tab === "ad" ? "bg-background shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}
						>
				...
				<CardContent className="space-y-4">
							...
							<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
								<div className="space-y-1.5">
									...
									<select
										...
										className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
									>
										{styles.map((s) => <option key={s.value} value={s.value}>{s.value}</option>)}
										{!styles.length && <option value="None">None</option>}
									</select>
								</div>
									...
									<div className="flex gap-1">
										{ASPECT_OPTIONS.map((a) => (
											<button
												...
												title={a.label}
												className={`flex-1 py-2 rounded-lg border text-sm transition-all ${aspect === a.value ? "border-primary bg-primary/10 text-primary font-medium" : "border-border hover:bg-muted text-muted-foreground"}`}
											>
												{a.icon}
											</button>
										))}
						...
						<>
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
								<div className="space-y-1.5">
									...
									<div className="flex gap-1">
										{ASPECT_OPTIONS.map((a) => (
											<button key={a.value} onClick={() => setAspect(a.value)} title={a.label} className={`flex-1 py-2 rounded-lg border text-sm transition-all ${aspect === a.value ? "border-primary bg-primary/10 text-primary font-medium" : "border-border hover:bg-muted text-muted-foreground"}`}>
												{a.icon}
											</button>
										))}
									</div>
								...
								<div className="space-y-1.5">
									<label className="text-sm font-medium">Model</label>
									<select value={activeModel} onChange={(e) => setModel(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring" disabled={configLoading}>
					...
					<Button className="w-full" onClick={handleGenerate} disabled={generating || !activeModel}>
						<Wand2 size={16} />
						{generating ? "Generating..." : credits !== null ? `Generate image — ${credits} credits` : "Generate image"}
					</Button>
				</CardContent>
			</Card>

			<Card>
				<CardHeader><CardTitle className="text-lg">Generated Images ({gallery?.total ?? 0})</CardTitle></CardHeader>
				<CardContent>
					{galleryLoading ? (
